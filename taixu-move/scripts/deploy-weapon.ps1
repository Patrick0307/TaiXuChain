# 部署武器合约脚本
# 使用方法: .\deploy-weapon.ps1

Write-Host "=== 部署武器合约 ===" -ForegroundColor Cyan
Write-Host ""
...
# 部署
Write-Host "部署武器合约..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "1. Package ID (武器合约地址)" -ForegroundColor Cyan
    Write-Host "2. WeaponMintCap ID (铸造权限)" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "下一步: 查看 武器合约使用指南.md" -ForegroundColor Yellow
}
